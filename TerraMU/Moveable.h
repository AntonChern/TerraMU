...
public:
	...
	virtual void setPosition(float x, float y, float z) { position = vec3(x, y, z); };
	virtual void setPosition(vec3 position) { this->position = position; };
	virtual void setScale(vec3 s) { scale = s; };
	virtual void setScale(float s) { scale = vec3(s); };
	virtual void setScale(float xScale, float yScale, float zScale) { scale = vec3(xScale, yScale, zScale); };
	...
	virtual void increasePosition(float dx, float dy, float dz) { position += vec3(dx, dy, dz); };
	virtual void increasePosition(vec3 dv) { position += dv; };
	virtual void increaseScale(vec3 ds) { scale += ds; };
	virtual void increaseScale(float ds) { scale += vec3(ds); };
	virtual void increaseScale(float dxScale, float dyScale, float dzScale) { scale += vec3(dxScale, dyScale, dzScale); };
	virtual void increaseRotationX(float dr) { rotationX += dr; };
	virtual void increaseRotationY(float dr) { rotationY += dr; };
	virtual void increaseRotationZ(float dr) { rotationZ += dr; };
